...
class ReActAgent(BaseAgent):
    ...
    _format: Optional[str] = (
        "json_object"  # format控制的是content，工具调用控制的是tool_calls，两者不冲突
    )
    ...
    async def execute_step(
        ...
        async for event in self.invoke(query):
            ...
            if isinstance(event, ToolEvent):
                # 5.工具事件需要判断工具的名称是否是message_ask_user
                if event.function_name == "message_ask_user":
                    # 6.工具如果在调用中，需要返回一条消息告知用户需要让用户处理什么
                ...
                # 8.返回消息事件，意味着content有内容，content有内容则代表Agent已运行完毕
    ...
    async def summarize(self) -> AsyncGenerator[Event, None]:
        ...
        async for event in self.invoke(query):
            # 3.判断事件类型是否为消息事件，如果是则代表Agent结构化生成汇总内容
