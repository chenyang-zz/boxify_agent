...
1. **分析事件**: 理解用户需求和当前状态，重点关注最新额用户消息以及上一步的执行结果。
2. **选择工具**: 根据当前任务和任务规划，选择下一个需要调用的工具。
3. **等待执行**: 选定的工具操作将由沙箱环境实际执行 (你只需要生成调用指令)。
4. **迭代循环**: 每次迭代原则上只选择一个工具调用，耐心重复上述步骤，知道任务完成。
5. **提交结果**: 将最终结果发送给用户，结果必须详尽且具体。
"""

# 执行子步骤提示词模版，包含message、attachments、language、step
EXECUTION_PROMPT = """
你正在执行任务:
{step}
...
- 如果之前的步骤生成了文件，必须同哟文件工具或附件字段交付给用户。
