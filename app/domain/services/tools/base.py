"""
Boxify工具设计思路
1.所有工具都必须继承自一个BaseTool基类，拥有同一的invoke方法用于调用该类下的对应工具;
2.定义一个装饰漆，被该装饰器装饰的方法会填充_tool_name,_tool_description,_tool_schema属性;
3.工具类可以通过get_tools快速获取基于缓存的schema参数信息，方便LLM调用;
4.LLM生成的内容可能会有幻觉，调用工具前需要筛选出LLM生成参数中符合工具的相关参数;
"""
...
class BaseTool:
    ...
    def _filter_parameters(
        ...
        """传递method+kwargs并过滤参数，使其符合method参数要求，因为LLM的输出内容可能存在幻觉"""
        # 1.定义一个变量用于存储过滤之后的字典信息
        filtered_kwargs = {}
    ...
    async def invoke(self, tool_name: str, **kwargs) -> ToolResult:
        ...
        for _, method in inspect.getmembers(self, inspect.ismethod):
            ...
            if (
                ...
                filterd_kwargs = self._filter_parameters(method, kwargs)

                # 4.调用方法获取工具结果
                return await method(**filterd_kwargs)
