...
from app.domain.services.tools.base import BaseTool, tool, ToolResult
...
class FileTool(BaseTool):
    ...
    @tool(
        name="write_file",
        description="对文件进行覆盖或追加写入。用于创建文件、追加内容或修改文件。",
        parameters={
            "filepath": {
                "type": "string",
            ...
            "trailing_newline": {
                "type": "boolean",
                "description": "(可选)是否追加后置换行符，在内容结尾",
            },
            "sudo": {"type": "boolean", "description": "(可选)是否使用 sudo 权限"},
        },
    ...
    async def write_file(
        ...
        """向文件中写入或追加内容"""
        return await self._sandbox.write_file(
            filepath=filepath,
            content=content,
            append=append,
    ...
    async def replace_in_file(
        ...
        """替换文件中的内容"""
        return await self._sandbox.replace_in_file(
            filepath=filepath,
            old_str=old_str,
            new_str=new_str,
            sudo=sudo,
    ...
    @tool(
        ...
        parameters={
            ...
            "glob_pattern": {
                "type": "string",
                "description": "使用 glob 语法通配符的文件命名模式",
            },
        },
    ...
    async def find_files(
        ...
        """在指定目录查找文件"""
        return await self._sandbox.find_files(
            dir_path=dir_path,
