...
class Step(BaseModel):
    ...
    id: str = Field(default_factory=lambda: str(uuid4()))  # 子任务id
    description: str = ""  # 步骤的描述信息
    status: ExecutionStatus = ExecutionStatus.PENDING  # 子任务的执行状态
    result: Optional[str] = None  # 子任务的执行结果
    error: Optional[str] = None  # 子任务的错误信息
    success: bool = False  # 子任务是否执行成功
    attachments: List[str] = Field(default_factory=list)  # 子任务的附件列表
...
class Plan(BaseModel):
    ...
    id: str = Field(default_factory=lambda: str(uuid4()))  # 计划id
    title: str = ""  # 任务标题
    goal: str = ""  # 任务目标
    language: str = ""  # 工作语言
