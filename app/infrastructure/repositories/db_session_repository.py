...
class DBSessionRepository(SessionRepository):
    ...
    async def delete_by_project(self, project_id: str, user_id: str) -> None:
        """删除当前用户指定项目下的所有会话"""
        stmt = delete(SessionModel).where(
    ...
    async def add_event(self, session_id: str, event: BaseEvent) -> None:
        ...
        stmt = (
            ...
            .where(SessionModel.id == session_id)
            .values(
                events=func.coalesce(SessionModel.events, orm_cast([], JSONB))
                + orm_cast([event_data], JSONB)
            )
        )
        result = await self.db_session.execute(stmt)
    ...
    async def add_file(self, session_id: str, file: File) -> None:
        ...
        stmt = (
            ...
            .where(SessionModel.id == session_id)
            .values(
                files=func.coalesce(SessionModel.files, orm_cast([], JSONB))
                + orm_cast([file_data], JSONB)
            )
        )
        result = await self.db_session.execute(stmt)
    ...
    async def save_memory(
        ...
        stmt = (
            ...
            .where(SessionModel.id == session_id)
            .values(
                memories=func.coalesce(SessionModel.memories, orm_cast({}, JSONB))
                + orm_cast(patch_data, JSONB),
            )
        )
        result = await self.db_session.execute(stmt)
