...
class RedisStreamTask(Task):
    ...
    def _cleanup_registry(self):
        ...
        if self._id in RedisStreamTask._task_registry:
            ...
            logger.info(f"任务[{self._id}]从任务中心移除")

    def _on_task_done(self):
        """任务结束时的回调函数"""
        # 1.检查task_runner是否存在，如果存在则调用task_runner的回调函数
    ...
    async def _execute_task(self):
        ...
        try:  #         except asyncio.CancelledError:
            await self._task_runner.invoke(
                self
    ...
    def cancel(self) -> bool:
        ...
        if not self.done:
            # 1.取消任务
            cast(asyncio.Task, self._execution_task).cancel()
